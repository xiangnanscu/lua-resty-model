$ resty -I lib -I resty_modules/lualib -I resty_modules/site/lualib --main-conf 'env NODE_ENV;' --http-conf 'lua_ssl_trusted_certificate /etc/ssl/certs/ca-certificates.crt;' -I spec bin/ngx_busted.lua -o TAP
...
VALUES
  (
    ...
    'john@example.com',
    30,
    '[{"end_date":"2020-01-01","company":"CompanyA","position":"Developer","description":"Workedonvariousprojects.","start_date":"2015-01-01"}]'
  )
```
...
VALUES
  (
    ...
    'jane@example.com',
    28,
    '[{"end_date":"2021-01-01","company":"CompanyB","position":"Designer","description":"Designeduserinterfaces.","start_date":"2016-01-01"}]'
  )
```
...
```sql
INSERT INTO entry AS T (blog_id, reposted_blog_id, headline, body_text, pub_date, mod_date, number_of_comments, number_of_pingbacks, rating) VALUES (1, DEFAULT, 'Third Entry', 'This is the third entry in my blog.', '2023-01-01', '2023-
01-02', 5, 2, 4)
```
...
VALUES
  ('Book One', 300, 29.99, 4.5, 1, 1, '2022-01-01')
```
...
DELETE FROM blog T
WHERE
  T.name LIKE 'bulk insert%' ESCAPE '\'
```
...
DELETE FROM blog T
WHERE
  T.name LIKE 'bulk insert return%' ESCAPE '\'
```
...
FROM
  entry T
WHERE
  T.headline LIKE '% from first blog' ESCAPE '\'
```
...
```lua
 Author:merge { { name2 = 'Tom', age = 11 } }:exec() 
```

```lua
 Author:merge({ { name = 'Tom', age = 11 } }, "name1"):exec() 
```
...
# merge_gets
## merge_gets with select first
```sql
INSERT INTO
  blog AS T (name, tagline)
VALUES
  ('First Merge Blog', 'First tagline')
RETURNING
  *
```
...
INSERT INTO
  blog AS T (name, tagline)
VALUES
  ('Second Merge Blog', 'Second tagline')
RETURNING
  *
```
...
Blog:select('name'):merge_gets({
  { id = 24, name = 'Merged First Blog' },
  { id = 25, name = 'Merged Second Blog' }
}, 'id'):exec()
...
WITH
  V (id, name) AS (
    VALUES
      (24::integer, 'Merged First Blog'::varchar),
      (25, 'Merged Second Blog')
  )
SELECT
  T.name,
  V.*
...
[
  {
    id  : 24,
    name: "Merged First Blog",
  },
  {
    id  : 25,
    name: "Merged Second Blog",
  },
]
```
...
ok 56 - merge_gets merge_gets with select first
## merge_gets with select last
```sql
INSERT INTO
  blog AS T (name, tagline)
VALUES
...
```lua
Blog:merge_gets({
  { id = 26, name = 'Merged First Blog' },
  { id = 27, name = 'Merged Second Blog' }
}, 'id'):select('name'):exec()
...
WITH
  V (id, name) AS (
    VALUES
      (26::integer, 'Merged First Blog'::varchar),
      (27, 'Merged Second Blog')
  )
SELECT
  V.*,
  T.name
...
[
  {
    id  : 26,
    name: "First Merge Blog",
  },
  {
    id  : 27,
    name: "Second Merge Blog",
  },
]
```
...
ok 57 - merge_gets merge_gets with select last
1..57
