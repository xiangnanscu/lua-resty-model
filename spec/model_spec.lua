...
local function contains_object(state, arguments)
  local a = arguments[1]
  local b = arguments[2]

  if not type(a) == "table" or not type(b) == "table" or #arguments ~= 2 then
    return false
  end

  for key_b, value_b in pairs(b) do
    if a[key_b] == nil or a[key_b] ~= value_b then
      return false
    end
  end
...
say:set("assertion.contains_object.positive", "Expected %s \nto contain: %s")
say:set("assertion.contains_object.negative", "Expected %s \nto not contain: %s")
assert:register("assertion", "contains_object", contains_object, "assertion.contains_object.positive",
  "assertion.contains_object.negative")

local function contains_error(state, arguments)
  local func = arguments[1]
  local expected = arguments[2]

  if not type(func) == "function" or #arguments ~= 2 then
    return false
  end
  ...
  if success then
    return false -- Expected an error, but none occurred
  end
  ...
  if err_type ~= expected_type then
    return false -- 类型不匹配
  end

  -- 根据类型使用不同的比较逻辑
  if err_type == "table" then
    -- 对于 table 类型，使用 contains_object 的逻辑
    for key_expected, value_expected in pairs(expected) do
      if err[key_expected] == nil or err[key_expected] ~= value_expected then
        return false
      end
    end
    return true
  elseif err_type == "string" then
    -- 对于 string 类型，使用原有的 string.find 逻辑
    return string.find(err, expected) ~= nil
  else
    -- 其他类型直接比较相等
    return err == expected
  end
...
say:set("assertion.contains_error.positive", "Expected function to error with message containing: %s")
say:set("assertion.contains_error.negative",
  "Expected function to not error with message containing: %s, but got error: %s")

assert:register("assertion", "contains_error", contains_error, "assertion.contains_error.positive",
  "assertion.contains_error.negative")
...
Blog:merge {
  ...
  mit("merge抛出异常的情况", function()
    ...
    assert.error(function()
      eval [[ Author:merge { { name2 = 'Tom', age = 11 } }:exec() ]]
    end, {
      batch_index = 1,
      message = '姓名不能为空',
      name = 'name',
    ...
    assert.error(function()
      eval [[ Author:merge({ { name = 'Tom', age = 11 } }, "name1"):exec() ]]
    end, 'invalid key for bulk operation: name1')
  end)
end)
...
mdesc("merge_gets", function()
  mit("merge_gets with select first", function()
    local first = Blog:create { name = 'First Merge Blog', tagline = 'First tagline' }
    local second = Blog:create { name = 'Second Merge Blog', tagline = 'Second tagline' }
    local res = eval(format([[
Blog:select('name'):merge_gets({
  { id = %d, name = 'Merged First Blog' },
  { id = %d, name = 'Merged Second Blog' }
}, 'id'):exec()
    ]], first.id, second.id))
    assert.are.same(res, {
    ...
    Blog:delete { id__in = { first.id, second.id } }:exec()
  end)

  mit("merge_gets with select last", function()
    local first = Blog:insert { name = 'First Merge Blog', tagline = 'First tagline' }:returning('*'):exec()[1]
    local second = Blog:insert { name = 'Second Merge Blog', tagline = 'Second tagline' }:returning('*'):exec()[1]
    local res = eval(format([[
Blog:merge_gets({
  { id = %d, name = 'Merged First Blog' },
  { id = %d, name = 'Merged Second Blog' }
    ...
    assert.are.same(res, {
      { id = first.id,  name = 'First Merge Blog' },
      { id = second.id, name = 'Second Merge Blog' }
    })
    Blog:delete { id__in = { first.id, second.id } }:exec()
  end)
end)
