...
local function main()
  ...
  local function eval(s, ctx)
    ...
    local res = { utils.eval(s,
      utils.dict(models, { models = models, Q = Q, F = F, Sum = Sum, Avg = Avg, Max = Max, Min = Min, Count = Count },
        ctx)) }
    local ins = res[1]
  ...
  mdesc("Xodel:where", function()
    ...
    mdesc("where author payload contains", function()
      local res = eval [[ Author:where { payload__contains = { status = 'active' } } ]]
