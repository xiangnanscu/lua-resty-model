...
---@field lazy? boolean
---@field tag? string
---@field get_model fun(AnyField?):Model
BaseField = class({}, {
...
---@field minlength? integer
---@field maxlength? integer
---@field image_base64? boolean whether this field stores base64 encoded image data
TextField = BaseField:class {
  option_names = { "pattern", "length", "minlength", "maxlength", "image_base64" },
}
...
function ForeignkeyField:init(options)
  ...
  if fk_model == "self" then
    -- used with Model._make_model_class
    return self
  end
  ---@cast fk_model Model
...
function ForeignkeyField:load(value)
  local fk_name = self.reference_column
  local fk_model = self.reference --[[@as Model]]
...
---@field compress? number|string 自动压缩图片到指定大小，如 "1M"、"200K"
---@field compress_arg? string
...
function AliossField.setup(self, options)
  ...
  if options.compress then
    self.compress_arg = options.compress
    self.compress = byte_size_parser(options.compress)
  end
end
