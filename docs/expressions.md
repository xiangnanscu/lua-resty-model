...
```lua
local Count = require("xodel.model").Count
local Sum   = require("xodel.model").Sum
local Avg   = require("xodel.model").Avg
local Max   = require("xodel.model").Max
local Min   = require("xodel.model").Min
```

### 用法

所有聚合函数通过 `annotate` 方法使用，接受一个字段名参数：
